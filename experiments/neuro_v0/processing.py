...
def processing_function(input_dict):
    ...
    for eeg_marker in ['TP9', 'TP10', 'AF7', 'AF8']:
        ...
        fs = int(round(1.0 / np.mean(np.diff(eeg_time)))) # Hz
        # fs = 256

        # Zapline (notch filter)
        ...
        for band, (low, high) in bands.items():
            ...
            band_power = np.trapezoid(psd[idx], f[idx])
...
if __name__ == '__main__':
    input_files = {
        'test_sample': r'C:\Users\victo\Local\repos\musepy\data\Muse_Data\resting_layed.data'
    }
